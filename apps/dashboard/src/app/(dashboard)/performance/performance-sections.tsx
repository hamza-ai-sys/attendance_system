            ...
            return (
              <article key={template.id} className="panel" style={{ cursor: "default" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: "12px" }}>
                  ...
                  <strong style={{ color: status.color, whiteSpace: "nowrap" }}>
                    {status.label}
                  </strong>
                </div>
                <dl
                ...
                >
                  ...
                  <dd>{template.fields.length} fields</dd>
                  <dt className="muted">Submitted</dt>
                  <dd>{template.evaluationCount} responses</dd>
                </dl>
                <button
                  type="button"
              ...
              {evaluations.map((evaluation) => (
                <tr key={evaluation.id}>
                  <td>
                    ...
                    <div className="muted">{evaluation.employeeEmail}</div>
                  </td>
                  <td>
                    {evaluation.evaluatorName}
                    <div className="muted">{evaluation.evaluatorRole}</div>
                  </td>
      ...
      <div className="modal-card" onClick={(event) => event.stopPropagation()}>
        ...
        <dl style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
          <div>
            ...
            <dd>{evaluation.employeeName}</dd>
          </div>
          <div>
            <dt className="muted">Evaluator</dt>
            <dd>
          ...
          <div>
            ...
            <dd>{evaluation.templateTitle}</dd>
          </div>
          <div>
            <dt className="muted">Overall score</dt>
            <dd>{evaluation.overallScore === null ? "N/A" : `★ ${evaluation.overallScore} / 5`}</dd>
          </div>
        </dl>
